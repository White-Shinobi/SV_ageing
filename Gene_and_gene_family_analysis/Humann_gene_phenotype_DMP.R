# Association analysis in DMP:Humann 4 Uniref90 (all genes from all SVs) ~ phenotypes
...
Humann4.0_DMP <- readRDS("/scratch/p303998/SV_MWAS/NM_revision/Humann4.0_DMP.rds")
...
sample_name = Reduce(intersect, list(row.names(pheno_cohort), row.names(Humann4.0_DMP), row.names(pheno_cov_abundance)))
covar = c("")
result = genes_correct_noSV(Humann4.0_DMP[sample_name,], pheno_cov_abundance[sample_name,6:113],sgv_full[sample_name,],
...
result = genes_correct_noSV(Humann4.0_DMP[sample_name,{NUM_RANGE}], pheno_cohort[sample_name,,drop=F],sgv_full[sample_name,],
                                pheno_cov_abundance[sample_name,], covar,colnames(Humann4.0_DMP)[{NUM_RANGE}])
for (i in c("Beta_IDage","SE_IDage","p_IDage", "Beta_phenotype","SE_phenotype","p_phenotype","N","y_uniq_N","x_uniq_N","y_non_zero_N","x_non_zero_N","y_non_zero_rate","x_non_zero_rate")){`result`[,i]=as.numeric(`result`[,i])}
saveRDS(result,paste0("/scratch/p303998/SV_MWAS/NM_revision/scripts_age/","_range_{NUM_RANGE}.rds"))
'
...
setwd("/scratch/p303998/SV_MWAS/NM_revision/scripts_age/")
generate_scripts(1, ncol(Humann4.0_DMP), 20)

#### 2.1.3 linux submit ####
# cd /scratch/p303998/SV_MWAS/NM_revision/scripts_age/
# vim submit_jobs.sh
#!/bin/bash
# SCRIPT_DIR="/scratch/p303998/SV_MWAS/NM_revision/scripts_age/" #####!!!!! Don't forget to change the folder name.
...
setpath <- "/scratch/p303998/SV_MWAS/NM_revision/scripts_age/"
...
saveRDS(combined_dataframe,paste0("/scratch/p303998/SV_MWAS/NM_revision/scripts_age/results/","Age_gene_results",".rds"))
...
result = genes_correct_noSV(Humann4.0_DMP[sample_name,{NUM_RANGE}], pheno_cohort[sample_name,,drop=F],sgv_full[sample_name,],
                                pheno_cov_abundance[sample_name,], covar,colnames(Humann4.0_DMP)[{NUM_RANGE}])
for (i in c("Beta_IDage","SE_IDage","p_IDage", "Beta_phenotype","SE_phenotype","p_phenotype","N","y_uniq_N","x_uniq_N","y_non_zero_N","x_non_zero_N","y_non_zero_rate","x_non_zero_rate")){`result`[,i]=as.numeric(`result`[,i])}
saveRDS(result,paste0("/scratch/p303998/SV_MWAS/NM_revision/scripts_sex/","_range_{NUM_RANGE}.rds"))
'
...
setwd("/scratch/p303998/SV_MWAS/NM_revision/scripts_sex/")
generate_scripts(1, ncol(Humann4.0_DMP), 20)
...
setpath <- "/scratch/p303998/SV_MWAS/NM_revision/scripts_sex/"
list = list.files(setpath,pattern = '.rds',recursive = F,full.names = TRUE)%>%unique()
dataframes_list <- lapply(list, readRDS)
combined_dataframe <- do.call(rbind, dataframes_list)
saveRDS(combined_dataframe,paste0("/scratch/p303998/SV_MWAS/NM_revision/scripts_sex/results/","Sex_gene_results",".rds"))
...
result = genes_correct_noSV(Humann4.0_DMP[sample_name,{NUM_RANGE}], pheno_cohort[sample_name,,drop=F],sgv_full[sample_name,],
                                pheno_cov_abundance[sample_name,], covar,colnames(Humann4.0_DMP)[{NUM_RANGE}])
for (i in c("Beta_IDage","SE_IDage","p_IDage", "Beta_phenotype","SE_phenotype","p_phenotype","N","y_uniq_N","x_uniq_N","y_non_zero_N","x_non_zero_N","y_non_zero_rate","x_non_zero_rate")){`result`[,i]=as.numeric(`result`[,i])}
saveRDS(result,paste0("/scratch/p303998/SV_MWAS/NM_revision/scripts_all_phenos/","_range_{NUM_RANGE}.rds"))
'
...
setwd("/scratch/p303998/SV_MWAS/NM_revision/scripts_all_phenos/")
generate_scripts(1, ncol(Humann4.0_DMP), 20)
...
setpath <- "/scratch/p303998/SV_MWAS/NM_revision/scripts_all_phenos/"
list = list.files(setpath,pattern = '.rds',recursive = F,full.names = TRUE)%>%unique()
dataframes_list <- lapply(list, readRDS)
combined_dataframe <- do.call(rbind, dataframes_list)
length(unique(combined_dataframe$X))#263
length(unique(combined_dataframe$Y))#40565
length(unique(combined_dataframe$X))*length(unique(combined_dataframe$Y))==nrow(combined_dataframe)#10668595
saveRDS(combined_dataframe,paste0("/scratch/p303998/SV_MWAS/NM_revision/scripts_all_phenos/results/","All_gene_results",".rds"))
#tmp=as.data.frame(table(combined_dataframe$Y[combined_dataframe$X==c("OtherMusculoskeletal")]))
#### 5. all results ####
Age_gene_results <- readRDS("/scratch/p303998/SV_MWAS/NM_revision/scripts_age/results/Age_gene_results.rds")
Sex_gene_results <- readRDS("/scratch/p303998/SV_MWAS/NM_revision/scripts_sex/results/Sex_gene_results.rds")
All_gene_results <- readRDS("/scratch/p303998/SV_MWAS/NM_revision/scripts_all_phenos/results/All_gene_results.rds")
...
saveRDS(Final_results,paste0("/scratch/p303998/SV_MWAS/NM_revision/","Final_results",".rds"))
